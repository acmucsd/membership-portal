...
export default class MerchStoreService {
  ...
  public async findItemByUuid(uuid: Uuid, user: UserModel): Promise<PublicMerchItemWithPurchaseLimits> {
    return this.transactions.readOnly(async (txn) => {
      ...
      const lifetimeCancelledItems = lifetimePurchaseHistory.filter((oi) => oi.order.status === OrderStatus.CANCELLED);
  ...
  public async getAllCollections(canSeeInactiveCollections = false): Promise<MerchandiseCollectionModel[]> {
    return this.transactions.readOnly(async (txn) => {
  ...
  public async createCollection(collection: MerchCollection): Promise<MerchandiseCollectionModel> {
    return this.transactions.readWrite(async (txn) =>
      Repositories.merchStoreCollection(txn).upsertMerchCollection(MerchandiseCollectionModel.create(collection)),
    );
  }

  public async editCollection(uuid: Uuid, collectionEdit: MerchCollectionEdit): Promise<MerchandiseCollectionModel> {
    return this.transactions.readWrite(async (txn) => {
      const merchCollectionRepository = Repositories.merchStoreCollection(txn);
      ...
      if (discountPercentage !== undefined) {
        await Repositories.merchStoreItemOption(txn).updateMerchItemOptionsInCollection(uuid, discountPercentage);
      }
      if (changes.archived !== undefined) {
        await Repositories.merchStoreItem(txn).updateMerchItemsInCollection(uuid, { hidden: changes.archived });
      }
  ...
  public async deleteCollection(uuid: Uuid): Promise<void> {
    return this.transactions.readWrite(async (txn) => {
      ...
      if (!collection) throw new NotFoundError('Merch collection not found');
      const hasBeenOrderedFrom = await Repositories.merchOrderItem(txn).hasCollectionBeenOrderedFrom(uuid);
      if (hasBeenOrderedFrom) {
        throw new UserError('This collection has been ordered from and cannot be deleted');
      }
  ...
  public async editItem(uuid: Uuid, itemEdit: MerchItemEdit): Promise<MerchandiseItemModel> {
    return this.transactions.readWrite(async (txn) => {
      ...
      if (updatedCollection) {
        const collection = await Repositories.merchStoreCollection(txn).findByUuid(updatedCollection);
        if (!collection) throw new NotFoundError('Merch collection not found');
  ...
  public async deleteItemOption(uuid: Uuid): Promise<void> {
    await this.transactions.readWrite(async (txn) => {
      ...
      if (hasBeenOrdered) {
        throw new UserError('This item option has been ordered and cannot be deleted');
      }

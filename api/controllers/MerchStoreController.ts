...
export class MerchStoreController {
  ...
  async getOneMerchCollection(
    ...
    return {
      error: null,
      collection: canSeeHiddenItems ? collection : collection.getPublicMerchCollection(),
    };
  }
  ...
  async createMerchCollectionPhoto(
    ...
    const collectionPhoto = await this.merchStoreService.createCollectionPhoto(params.uuid, {
      uploadedPhoto,
      position,
    });
  ...
  async createMerchItemPhoto(
    ...
    const merchPhoto = await this.merchStoreService.createItemPhoto(params.uuid, { uploadedPhoto, position });
  ...
  async getMerchOrdersForAllUsers(@AuthenticatedUser() user: UserModel): Promise<GetMerchOrdersResponse> {
    if (!(PermissionsService.canAccessMerchStore(user) && PermissionsService.canSeeAllMerchOrders(user)))

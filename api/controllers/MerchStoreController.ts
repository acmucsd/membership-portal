...
export class MerchStoreController {
  ...
  constructor(
    merchStoreService: MerchStoreService,
    merchOrderService: MerchOrderService,
  ...
  async getOneMerchCollection(
    ...
    return {
      error: null,
      collection: canSeeHiddenItems
        ? collection
        : collection.getPublicMerchCollection(),
    };
  }
  ...
  async editMerchCollection(
    ...
    const collection = await this.merchStoreService.editCollection(
      params.uuid,
      editCollectionRequest.collection,
    );
    return { error: null, collection };
  }
  ...
  async createMerchCollectionPhoto(
    @UploadedFile('image', {
    ...
    const collectionPhoto = await this.merchStoreService.createCollectionPhoto(
      params.uuid,
      { uploadedPhoto, position },
    );

    return {
      error: null,
      collectionPhoto: collectionPhoto.getPublicMerchCollectionPhoto(),
    };
  }
  ...
  async createMerchItem(
    ...
    const item = await this.merchStoreService.createItem(
      createItemRequest.merchandise,
    );
    return { error: null, item };
  }
  ...
  async createMerchItemOption(
    ...
    const option = await this.merchStoreService.createItemOption(
      params.uuid,
      createItemOptionRequest.option,
    );
  ...
  @Get('/orders/all')
  async getMerchOrdersForAllUsers(
    @AuthenticatedUser() user: UserModel,
  ...
  async placeMerchOrder(
    ...
    const order = await this.merchOrderService.placeOrder(
      originalOrder,
      user,
      placeOrderRequest.pickupEvent,
    );
  ...
  private validateMerchOrderRequest(
  ...
  async fulfillMerchOrderItems(
    ...
    const updatedOrder = await this.merchOrderService.fulfillOrderItems(
      fulfillOrderRequest.items,
      params.uuid,
      user,
    );
  ...
  async getOnePickupEvent(
    ...
    return {
      error: null,
      pickupEvent: pickupEvent.getPublicOrderPickupEvent(true),
    };
  }
  ...
  async getCartItems(
    ...
    return {
      error: null,
      cart: cartItems.map((option) => option.getPublicOrderMerchItemOption()),
    };
  }
}
